...
try:
    local_master_addresses = open("%s/masterseed.txt" % config.data_dir, "r").read()
...
class UiRequestPlugin(object):
    ...
    def actionWrapper(self, path, extra_headers=None):
        ...
        if config.multiuser_no_new_sites:
            path_exceptions = ["merged", "Me.ZeroNetwork.bit"]
            path_check = any(path_exception in inner_path for path_exception in path_exceptions)
            if path_check is False:
                path_parts = self.parsePath(path)
                if not self.server.site_manager.get(match.group("address")) and (not user or user.master_seed not in local_master_addresses):
                    self.sendHeader(404)
        ...
        if not back_generator:  # Wrapper error or not string returned, injection not possible
            ...
            """.replace("\t", "")
            if user.master_seed in local_master_addresses:
...
class UiWebsocketPlugin(object):
    ...
    def formatServerInfo(self):
        ...
        if "ADMIN" in self.site.settings["permissions"]:
            ...
            is_multiuser_admin = config.multiuser_local or self.user.master_seed in local_master_addresses
    ...
    def hasCmdPermission(self, cmd):
        ...
        is_public_proxy_user = not config.multiuser_local and self.user.master_seed not in local_master_addresses
    ...
    def actionCertAdd(self, *args, **kwargs):
        ...
        message = """
            ...
            and help to keep the network alive, then please run your own <a href='https://zeronet.dev' target='_blank'>ZeroNet client</a> -- and then simply swap the "master_seed": "XXX.." in ZeroNet/data/users.json (line 12) with the one you were given.</small>
        """
