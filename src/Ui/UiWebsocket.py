...
class UiWebsocket(object):
    ...
    def actionCertSelect(self, to, accepted_domains=[], accept_any=False, accepted_pattern=None):
        accounts = []
        accounts.append(["", _["Guest (No Certificate)"], ""])  # Default option
        ...
        # Render the html
        body = "<span style='padding-bottom: 5px; display: block'>" + _["Create a new account or stay as guest:"] + "</span>"
        # Accounts
        ...
        if more_domains:
            ...
            body += "<div style='background-color: #F7F7F7'>"
            for domain in more_domains:
                body += _("""
                 <a href='/{domain}' target='_top' class='select'>
                  <small style='float: right; margin-right: 40px; margin-top: -1px'>{_[Certify or Create New Account]} &raquo;</small>{domain}
                 </a>
                """)
            body += "</div>"
            body += "<div style='background-color: #F7F7F7'>"
            body += _("""
             <a href='/1HELLoE3sFD9569CLCbHEAVqvqV7U2Ri9d' target='_top' class='select'>
              <small style='float: right; margin-right: 40px; margin-top: -1px'>{_[Login To An Existing Account]} &raquo;</small>ZeroNet Dashboard
             </a>
            """)
            body += "</div>"
